...
s_max_episode_num=7000
s_num_trials=15
s_max_steps = 100
s_numsteps = np.zeros((s_num_trials,s_max_episode_num), dtype='int')
s_avg_numsteps = np.zeros((s_num_trials,s_max_episode_num))

s_traj_data_x=np.ones((s_num_trials,s_max_episode_num,s_max_steps))
s_traj_data_y=np.ones((s_num_trials,s_max_episode_num,s_max_steps))
s_traj_data_derivative_x=np.ones((s_num_trials,s_max_episode_num,s_max_steps))
s_traj_data_derivative_y=np.ones((s_num_trials,s_max_episode_num,s_max_steps))
s_traj_data_phi=np.zeros((s_num_trials,s_max_episode_num,s_max_steps))
s_traj_data_dot_phi=np.ones((s_num_trials,s_max_episode_num,s_max_steps))

s_unsafe_flag_data=np.zeros((s_num_trials,s_max_episode_num))
...
for trial in range(s_num_trials):
    ...
    for episode in range(s_max_episode_num):
        ...
        for steps in range(s_max_steps):
            # env.render()
            state = system.vector_state()
            s_traj_data_x[trial][episode][steps] = state[0]
            s_traj_data_y[trial][episode][steps] = state[1]
            s_traj_data_phi[trial][episode][steps] = system.phi_val()
            ...
            if (t):
                ...
                s_numsteps[trial][episode] = int(steps + 1)
                for j in range(10):
                    s_avg_numsteps[trial][episode] += s_numsteps[trial][episode - j]
                s_avg_numsteps[trial][episode] = s_avg_numsteps[trial][episode] / 10
                if (episode<10):
                    s_avg_numsteps[trial][episode]=s_numsteps[trial][episode]
                if episode % 1 == 0:
                    sys.stdout.write(
                        "trial: {}, episode: {}, total reward: {}, length: {}\n".format(trial,
                                                                                        episode,
                                                                                        np.round(np.sum(rewards),
                                                                                                 decimals=3),
                                                                                        steps + 1))
        ...
        s_unsafe_flag_data[trial][episode] = s_unsafe_flag

    #plt.plot(numsteps[trial], label='#steps till destination/horizon')
    #plt.plot(avg_numsteps[trial], label='average #steps for last 10 episodes')
    #plt.xlabel('Episode')
...
np.save('safe_simulation_data',s_traj_data_phi)
np.save('safe_numsteps',s_numsteps)
np.save('safe_average_numsteps',s_avg_numsteps)
np.save('safe_simulation_traj_x',s_traj_data_x)
np.save('safe_simulation_traj_y',s_traj_data_y)
np.save('safe_num_trials',s_num_trials)
np.save('safe_max_episode_num',s_max_episode_num)
np.save('safe_max_steps',s_max_steps)
