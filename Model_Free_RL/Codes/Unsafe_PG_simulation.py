...
unsafe_flag_data=np.zeros((num_trials,max_episode_num))
...
for trial in range(num_trials):
    ...
    for episode in range(max_episode_num):
        ...
        for steps in range(max_steps):
            # env.render()
            state = system.vector_state()
            traj_data_x[trial][episode][steps] = state[0]
            traj_data_y[trial][episode][steps] = state[1]
            traj_data_phi[trial][episode][steps] = system.phi_val()
            ...
            if (t):
                ...
                numsteps[trial][episode] = int(steps + 1)
                for j in range(47):
                    avg_numsteps[trial][episode] += numsteps[trial][episode - j]
                avg_numsteps[trial][episode] = avg_numsteps[trial][episode] / 47
                if (episode<47):
                    avg_numsteps[trial][episode]=numsteps[trial][episode]
                if episode % 1 == 0:
                    sys.stdout.write(
        ...
        unsafe_flag_data[trial][episode] = unsafe_flag
...
my_path = os.path.abspath("/Users/ritabrataray/Desktop/Safe control/Model_Free_RL/Data/Unsafe")
    #plt.savefig(os.path.join(my_path, "Keynote_safe_learning.png"), format="png", bbox_inches="tight")
    #plt.show()


np.save(os.path.join(my_path,'unsafe_simulation_data'),traj_data_phi)
np.save(os.path.join(my_path,'unsafe_numsteps'),numsteps)
np.save(os.path.join(my_path,'unsafe_average_numsteps'),avg_numsteps)
np.save(os.path.join(my_path,'unsafe_simulation_traj_x'),traj_data_x)
np.save(os.path.join(my_path,'unsafe_simulation_traj_y'),traj_data_y)
